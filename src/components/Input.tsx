  ...
  return (
    <div className="w-full relative">
      <input
        id={id}
        tabIndex={0}
        className={`input cursor-text transition-colors dark:bg-dark-secondary/[.87] dark:text-dark-primary-900 font-medium px-3 py-2 w-full rounded-md ${inputWithHelperStyle} ${className} dark:focus:bg-dark-secondary shadow-lg`}
        type="text"
        value={value}
        onChange={(event) => {
        ...
        placeholder={' '}
      />
      <p className="absolute top-2 left-3 font-medium cursor-text dark:text-dark-primary-400/70 unclickable transition-all duration-200 placeholder">{placeholder}</p>

      <div className={`absolute z-10 dark:bg-dark-primary-600/50 w-full rounded-b-lg px-3 py-2 flex flex-col mt-1 font-bold text-sm ${visibility(showHelper)}`}>
